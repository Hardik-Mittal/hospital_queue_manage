...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        echo $username_err;
        header("refresh:0.5; url = http://localhost/login/register.php");

    }
    // else if(isset($_POST["username"])){
    //   $sq = "SELECT * FROM users WHERE username = '".$_POST["username"]."'";
    //   $reslt =mysqli_query($conn, $sq);
    //   if(mysqli_num_rows($reslt) > 0){
    //     echo '<span class="text-danger">Username not available</span>';
    //     header("refresh:0.7; url = http://localhost/login/register.php");
    //   }
    //   else{
    //     echo '<span class="test-successs">Username available</span>';
    //   }
    // }
    else{
...
<html lang="en">
  ...
  <ul class="navbar-nav">
      <li class="nav-item active">
        <a class="nav-link" href="http://localhost/login/index.php">Home <span class="sr-only">(current)</span></a>
      </li>
      <li class="nav-item">
